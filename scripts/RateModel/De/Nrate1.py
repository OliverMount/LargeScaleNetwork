# Here we use fI for the inhibitory as well; Then this will be exactly the Dermirtas model
# Nrate.py contains codes only for the Chaudri
# This also includes finding steady-state values for the inhibitory
...
print("MAX :",np.amax(FLN_MTX))
print("MIN :",np.amin(FLN_MTX))
...
# This ifunction solves for the steady-state values,  that is, ds/dt=0
def _solver(s):
	
	r_inh = p['beta_inh'] * (p['exc_scale'] * p['wIE'] *s + I_bkg_inh / p['beta_inh']) / (1 + p['beta_inh']* p['wII'])  # Long range is ignored here though it is mentioned in the paper
...
#def calc_bg():
#	"""
#	This function solves for baseline gating variable and firing rates
#	"""	
...
#print(s_tgt)
# The above function solves the residual (that is, given the residual find s)
