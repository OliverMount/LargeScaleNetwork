...
print("The current in nA for the rate of ", round(r_e,2) , "is ",round(I_e,4), "nA")


status_message("Solving inhibitory current (I_i)")
...
def solve4_inh_synaptic_current(inh):
	res =np.array([(wI*Ib) + (wEI * se)-tauI*fI(aI,bI,dI,k)-k  for k in inh])
...
print("The FIC inhibitory current is ", Ii_optimal)

# Balanced weight for inhibition control 
si=tauI*fI(aI,bI,dI,Ii_optimal) 
wIE_nsc=(-I_e +(wE*Ib) + (wEE*se)+(g*J*se))/(si)  				    # Without accounting for SC (what Dermirtas did)
wIE=(-I_e +(wE*Ib) + (wEE*se)+(g*J*np.dot(SC_scaled,sE)))/(si)  	# With accounting for SC  (what Deco did)

print("Mean Optimal wIE for FIC (with SC) is ", wIE.mean())
print("Optimal wIE for FIC (without SC) is ", wIE_nsc)
...
for t in range(1,nt):
	...
	d_SE = (-SE[t-1]/tauE) +((1-SE[t-1])*gamma*RE[t-1])+(np.sqrt(dt)*noi_sd*np.random.normal(size=(nROI)))  
	d_SI = (-SI[t-1]/tauI) +RI[t-1]+  (np.sqrt(dt)*noi_sd*np.random.normal(size=(nROI)))  
	...
	# For FIC 
	#wIE=-IE.mean() +(wE*Ib) + (wEE*SE[t].mean())+(g*J*SE[t].mean())/(SI[t].mean())  # Finding the inhibition control explicitly
	#print(wIE)
