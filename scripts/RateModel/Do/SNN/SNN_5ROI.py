...
# Each column is a time series of membrane current with no synaptic input
...
# Structural connectivity matrix (N_ROI X N_ROI matrix)
C=np.array([[0.        , 0.00030132, 0.00051185, 0.00065894, 0.00076171],
       [0.00090679, 0.        , 0.00044746, 0.00069469, 0.0008313 ],
       [0.00099953, 0.00099996, 0.        , 0.00092227, 0.00099396],
       [0.00099685, 0.00099982, 0.00099999, 0.        , 0.0009439 ],
       [0.00032689, 0.00054692, 0.00069502, 0.00079472, 0.        ]])

#ROI_names=np.repeat("ROI-",5) 

# For making random structural connectivity

#C=np.zeros((N_ROI,N_ROI))
#for l in range(N_ROI):
#    C[l,:]=roll(abs((1-np.exp(-(arange(N_ROI)*randn()/0.5)))*0.001),l)


# Form local synpatic binary connection matrix according to p_local
# For a single ROI , for multiple ROIs extend it

#import random
#Syn_connectivity = np.zeros((N_pool,N_pool))

#for l in range(N_pool):
#    temp=random.sample(range(N_pool), M_local)   # M_local contains p_local
#    Syn_connectivity[l,temp]=1



#figure(figsize=(10,10))
#imshow(Syn_connectivity)
#show()
 

### Defining input current time series for all the ROIs

# the mean and variance are chosen such that the input current ranges from 0.3 to 0.9 nA
me_input = 0.22   # in nano amp
var_input = 0.01 
(a,b,d)=IFpara(name="Derm_E")
Curr=np.zeros((sig_len,N_pool,N_ROI))  # N X N_pool X N_ROI
A=np.zeros((N,N_pool,N_ROI))
True_rate = np.zeros((N,N_ROI))

for l in range(N_ROI):
    if (l==0): # This is for desired ROI
        I = me_input + (0.5*0.1)*(np.sin(2*np.pi*20*Hz*array(range(N))*dt) + np.sin(2*np.pi*25*Hz*array(range(N))*dt)) 
        Curr[:,:,l],_,A[:,:,l]= SynapticKernel(I,N_pool,shift,var_input=var_input,type="LIF")   # Curr is for one whole ROI output and A is input
        Curr[:,N_e:,l]=-Curr[:,N_e:,l] 			# For inhibitory
        True_rate[:,l]=IFcurve(a,b,d,I*namp)  	# True rate (from IF curve)
    else:  										# For other ROI (more variation in the other ROI)
        I = (me_input-0.035) + (0.5*0.1)*(np.sin(2*np.pi*(10+(1.5*l))*Hz*array(range(N))*dt) + np.sin(2*np.pi*30*Hz*array(range(N))*dt)) 
        Curr[:,:,l],_,A[:,:,l]= SynapticKernel(I,N_pool,shift,var_input=var_input,type="LIF")
        Curr[:,N_e:,l]=-Curr[:,N_e:,l] 			# For inhibitory 
        True_rate[:,l]=IFcurve(a,b,d,I*namp)  	# True rate (from IF curve)
...
start=time.time()
Local_con=np.zeros((N_pool,N_pool,N_ROI))
for l in range(N_ROI):
...
start=time.time()
Lrange_con=np.zeros((N_pool,N_e,N_ROI-1,N_ROI))
for l in range(N_ROI):
...
for roi in range(N_ROI):    # For each ROI 
    ...
    for Ind in range(N_pool):           # For each neuron in the pool  
        ...
        # For Inhibitory  neurons    
            ...
            WIE.append(W[:Lex])
            WII.append(W[Lex:])  # Negative Weights here(negated during synapse)
            W_mtx[Ind,Synaptic_Index,roi]=np.concatenate((W[:Lex],-W[Lex:]))
...
print("EE: ",len([item for sublist in Wroi[0] for item in list(sublist)]))
print("EI: ", len([item for sublist in Wroi[3] for item in list(sublist)]))
print("IE: ", len([item for sublist in Wroi[2] for item in list(sublist)]))
print("II: ",len([item for sublist in Wroi[1] for item in list(sublist)]))
...
for ax, area_idx in zip(ax_list, area_idx_list):
    ...
    y_plot = y_plot - y_plot.min()
    ax.plot(Spk[area_idx][:,0],Spk[area_idx][:,1]-((area_idx)*N_e)+5,'r|')
...
f.text(0.01, 0.5, 'Neuron Index', va='center', rotation='vertical',size=16)
f.set_size_inches(18.5, 10.5)
ax.set_xlabel('Time (ms)',size=16)
show()
...
df=pd.DataFrame(np.zeros((N_ROI,4)),index=ROI_names,columns=["WEE","WII","WIE","WEI"])
df 

for k in range(N_ROI):
    #print("For ROI-"+str(k+1)) 
    temp=[l for l in Wroi[k+ (k*(3))][0] if l!=0.0]
    df.iloc[k,0]=round(mean(temp),4)
    #print("WEE: ",df.iloc[k,0])
    temp=[l for l in Wroi[k+ (k*(3))+1][0] if l!=0.0]
    df.iloc[k,1]=round(mean(temp),4)
    #print("WII: ",df.iloc[k,1])
    temp=[l for l in Wroi[k+ (k*(3))+2][0] if l!=0.0]
    df.iloc[k,2]=round(mean(temp),4)
    #print("WIE: ",round(mean(temp),4))
    temp=[l for l in Wroi[k+ (k*(3))+3][0] if l!=0.0]
    df.iloc[k,3]=round(mean(temp),4)
    #print("WEI: ",round(mean(temp),4))
    #line_print(20)  
...
Wee=[l for l in WEE if l!=0.0]
print("Mean Wee:", mean(Wee))
Wii=[-l for l in WII if l!=0.0]
print("Mean Wii:", mean(Wii))

Wie=[l for l in WIE if l!=0.0]
print("Mean Wie:", mean(Wie))

Wei=[-l for l in WEI if l!=0.0]
print("Mean Wei:", mean(Wei)) 


""" figure(figsize=(20,3))
plot(S.t/ms,S.i,'r|',markersize=2)
#plot(S.t/ms,S.i[S.i==Ind],'r|',markersize=2)
xlim(0,1000)
#ylim(1800-tn,1800)
show()
"""
...
""" figure(figsize=(20,3))
plot(0.4 + (0.5*0.1)*(np.sin(2*np.pi*20*Hz*array(range(N))*dt) + np.sin(2*np.pi*25*Hz*array(range(N))*dt)) )
xlim(0,1000)
ylabel("Current (nA)")
xlabel('Time (msec)')
title("Current time series (toy example)")
show()
...
xlabel('Time (msec)')
title("Rate time series (Toy example): Rate obtained by Dermirtas IF curve")
show()
...
ylabel("Neuron Index")
xlabel('Time (msec)')
title("Raster plot of excitatory neurons")
xlim(0,1000)
#ylim(1800-tn,1800)
show()
"""
