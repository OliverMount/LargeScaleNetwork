...
load InputData/DKatlas_timeseries.mat
load InputData/DKcortex_selectedGenes.mat;
%coefi=sum(expMeasures(:,2:6),2); %% gaba
...
imagesc(FCtdata);colormap('jet');colorbar;
...
for nsub=1:NSUBSIM
    ...
    nn=nn-1;
    disp('Solved for one participant');
%end
   %%%% BOLD empirical
    ...
    %imagesc(squeeze(FCt2(nsub,:,:)));colormap('jet');colorbar;
...
%end  % for main loop
FCtsim=squeeze(nanmean(FCt2,1));  % FCt2 is the  simulation data (NsubX nROI X nROI)
...
%%%
save(sprintf('Wgain_%03d.mat',s),'GBCfitt','FCfitt','FCDfitt');
